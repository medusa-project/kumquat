namespace :dls do
  ...
  namespace :items do
    ...
    task :fix_child_titles => :environment do
      ...
      results.each do |row|
        ...
        if item.parent_repository_id.present?
          ...
          if pres_master
            ...
            raise "Item #{item.repository_id} has no preservation master binary"
          end
        else
          parents_missing_titles << item.repository_id
        end
      end
      puts "Added #{num_added} titles"
      puts "Parents missing titles:"
      parents_missing_titles.each do |item_id|
        puts item_id
      end
    end
