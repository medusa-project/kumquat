...
# consuming and involves the `opensearch:items:reindex`,
# `opensearch:collections:reindex`, and `elasticsearch:agents:reindex` rake
# tasks.
#
# Once the new index has been populated, either the application's
# `opensearch_index` configuration key must be updated to point to it, or else
# the index alias that that key is pointing to must be changed to point to the
# new index.
#
