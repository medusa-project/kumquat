class LandingController < WebsiteController
  ...
  def index
    ...
    @random_item = Item.solr.where(Item::SolrFields::PUBLISHED => true).
    ...
    @num_all_items = Item.solr.all.count
    @num_top_level_items = Item.solr.where(Item::SolrFields::PARENT_ITEM => :null).count

    @num_audio_items = Item.solr.where("#{Item::SolrFields::ACCESS_MASTER_MEDIA_TYPE}:audio/* "\
    "OR #{Item::SolrFields::PRESERVATION_MASTER_MEDIA_TYPE}:audio/*").count

    doc_media_types = %w(application/pdf text/plain).join(' OR ')
    @num_document_items = Item.solr.where("#{Item::SolrFields::ACCESS_MASTER_MEDIA_TYPE}:(#{doc_media_types}) "\
    "OR #{Item::SolrFields::PRESERVATION_MASTER_MEDIA_TYPE}:(#{doc_media_types})").count

    @num_image_items = Item.solr.where("#{Item::SolrFields::ACCESS_MASTER_MEDIA_TYPE}:image/* "\
